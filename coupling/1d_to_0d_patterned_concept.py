...
# coupling
S01 = Synapses(G1, G0, 'w : volt', delay=3*ms, on_pre='v+=w')
S01.connect()
s01_center = 60
S01.w = '(2.0 * exp(-(s01_center-i)**2/(2*(15.0**2)))) * mV'


# input
input_center = 20.0
input_strength = 2000.0
P = PoissonGroup(N1, '(input_strength * exp(-(input_center-i)**2/(2*(6.0**2))))* Hz')
S1P = Synapses(P, G1, on_pre='v+=1.0 * mV')
S1P.connect(j='i')
...
# plotting
figure()
suptitle('Poisson input into field')
plot(MP.t/ms, MP.i, '.k')
xlabel('Time (ms)')
ylabel('Input index')
xlim(0,runtime/ms)
...
figure()
suptitle('Connection weights from field to node')
brian_plot(S01.w)
show()
